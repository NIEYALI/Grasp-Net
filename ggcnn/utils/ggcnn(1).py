...
def input_img(img, out_size=300):
    ...
    assert img.shape[0] >= out_size and img.shape[1] >= out_size, '输入的深度图必须大于等于(360, 360)'
...
class GGCNN:
    ...
    def predict(self, img, mode, thresh=0.3, peak_dist=1):
        ...
        :return:
            ...
            crop_y1
        """
        # input为原图裁剪之后的图像，维度为1x1x300x300，crop_x1,crop_y1为裁剪的左上角像素坐标
